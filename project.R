...
fun2<-function(x) {
  if (x[1]+x[2]>=1 ||x[1]<0||x[2]<0) {
    ...
    for (i in 2:250) {
      for (j in 1:30) {
        for (k in j:30) {
          q_matrix[j,k,i]=1+x[1]*(z_matrix[j,k,(i-1)]-1)+x[2]*(q_matrix[j,k,(i-1)]-1)
        }
      }
    }
...
for (i in 2:250) {
  for (j in 1:30) {
    for (k in j:30) {
      q_matrix[j,k,i]=1+alpha_rho*(z_matrix[j,k,(i-1)]-1)+beta_rho*(q_matrix[j,k,(i-1)]-1)
    }
  }
}
...
####option prices
option_price<-read.xls("~/R/FRM/options.xlsx")
call_buy<-option_price[1,]
call_sell<-option_price[2,]
put_buy<-option_price[3,]
put_sell<-option_price[4,]
...
value0=as.numeric(-call_sell[31]-put_sell[31])
for (i in 1:30) {
  value0=value0+as.numeric(kw[i]*(call_buy[i]+put_buy[i])) 
}
...
for (i in 1:n_sim) {
  call_dji_sim[i]=100*GBSOption(TypeFlag = "c",S=dji_sim[i]/100,X=dji_price[251]/100,
                Time=tau-1/252,r=interest,b=interest-yield[31],sigma = dji_vol[250]*sqrt(252))@price
}
...
for (i in 1:30) {
  ...
  theta_matrix[i,2]=GBSGreeks(Selection = "theta",TypeFlag = "p",S=stock_price[251,i],X=stock_price[251,i],
                              Time=tau,r=interest,b=interest-yield[i],sigma = stock_vol[250,i]*sqrt(252) )
  theta_matrix[i,3]=kw[i]*(gamma_matrix[i,1]+gamma_matrix[i,2])
}
...
for (i in 1:30) {
  ...
  rho_matrix[i,2]=GBSGreeks(Selection = "rho",TypeFlag = "p",S=stock_price[251,i],X=stock_price[251,i],
                              Time=tau,r=interest,b=interest-yield[i],sigma = stock_vol[250,i]*sqrt(252) )
  rho_matrix[i,3]=kw[i]*(gamma_matrix[i,1]+gamma_matrix[i,2])
}
